...
スコープへ自動的にバインドされます。React が削除した子孫要素のバインディングも
自動的に破棄されます。後から適用されるこれらのバインディングで発生した例外も、最も近い
React Error Boundary に届きます。React が既存要素の `data-bind` 属性を変更した場合は、以前の
バインディングを破棄し、同じ最寄りのルートまたはスコープで新しい式を適用します。`text`、
`html`、`component`、`options` バインディングを取り除く場合は、現在のバインディングまたは
React が描画した children に所有権を渡す前に、Knockout が作成した内容も削除します。
