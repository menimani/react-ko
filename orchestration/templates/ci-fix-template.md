...
## Procedure
1. Read the failing job's log. Reproduce with the command CI itself ran — for this
   repository that is `npm run test` and `npm run build` from the root. The build is the
   type gate: a suite that passes while the build fails is still a failure.
...
3. **Reproduce the failure locally first.** A test that fails only in CI usually fails on
   timing, environment or dependency versions rather than on anything the diff shows, and
   a run that passes on your machine is not evidence it is fixed — it is the state it was
   already in. Force the condition: delay the mock the test waits on, run the file alone
   and run it with the full suite, reinstall from the lockfile with `npm ci`. If you
   cannot make it fail, say so and describe what you tried rather than claiming a fix.
...
- English only, in code, comments, tests and commit messages alike.
