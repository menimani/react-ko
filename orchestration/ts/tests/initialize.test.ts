...
import { loadProject, renderProjectAdapter } from '../src/adapters/project.ts'
...
describe('repository initialization', () => {
  ...
  it('leaves contract-complete project files unchanged when repaired', async () => {
    const { repository, installedPackage } = fixture()
    const forge = makeFakeForge()
    ...
    const customizedAdapter = `${readFileSync(adapter, 'utf8')}\n// project-owned note\n`
    writeFileSync(adapter, customizedAdapter)
    ...
    expect(result.ok).toBe(true)
    expect(readFileSync(adapter, 'utf8')).toBe(customizedAdapter)
    expect(readFileSync(scan, 'utf8')).toBe('custom scan\n')
    expect(reports).toContain(`EXISTS: ${adapter} (contract complete; left unchanged)`)
    expect(reports).toContain(`EXISTS: ${scan} (left unchanged)`)
  })

  it('adds only a missing contract member and reports its name', async () => {
    const { repository, installedPackage } = fixture()
    const paths = orchPaths(repository)
    const projectDirectory = join(repository, 'orchestration', 'project')
    const adapter = join(projectDirectory, 'project-consumer.ts')
    const complete = renderProjectAdapter('consumer', '../ts/src/adapters/project.ts')
      .replace('  mergeChecks: () => [],', '  mergeChecks: () => customMergeChecks,')
      .replace('  preCommitChecks: [],\n\n', '')
      .replace(
        "export const project: ProjectAdapter = {",
        "const customMergeChecks = [{ label: 'Custom gate', cwd: '', command: 'npm test' }]\n\nexport const project: ProjectAdapter = {",
      )
    await initializeRepository(paths, makeFakeForge(), 'consumer', {
      packageRoot: installedPackage,
    })
    writeFileSync(adapter, complete)
    const reports: string[] = []
    ...
    const repaired = readFileSync(adapter, 'utf8')
    const generatedAddition = [
      "  // GENERATED by init: replace with this project's real values.\n",
      '  preCommitChecks: [],\n',
    ].join('')
    expect(result.ok).toBe(true)
    expect(repaired).toContain(generatedAddition)
    expect(repaired.replace(generatedAddition, '')).toBe(complete)
    expect(reports.filter((line) => line.startsWith('REPAIRED:'))).toEqual([
      `REPAIRED: ${adapter}; added required member 'preCommitChecks'`,
    ])
    await expect(loadProject(join(repository, 'orchestration'), {})).resolves.toMatchObject({
      name: 'consumer',
      preCommitChecks: [],
